...
def get_valid_stations():
    ...
    query = """
        SELECT DISTINCT id
        FROM `decisive-studio-469008-m2.LondonBicycles_Raw.cycle_stations_raw`
    ...
    valid_stations = [row.id for row in results]
...
if "cycle_hire_raw" not in datasource.get_asset_names():
    datasource.add_query_asset(
        name="cycle_hire_raw",
        query="SELECT * FROM `decisive-studio-469008-m2.LondonBicycles_Raw.cycle_hire_raw`"
    )
    print("Asset 'cycle_hire_raw' registered under datasource!")

# 4️⃣ Build batch request from Fluent asset
asset = datasource.get_asset("cycle_hire_raw")
batch_request_hire = asset.build_batch_request()

# 5️⃣ Create or update an expectation suite
suite_name = "cycle_hire_raw_suite"
...
checkpoint = context.add_or_update_checkpoint(
    name="cycle_hire_raw_checkpoint",
    validations=[
        {
            "batch_request": batch_request_hire,
            "expectation_suite_name": suite_name,
